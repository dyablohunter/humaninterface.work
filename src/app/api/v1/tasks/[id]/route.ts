...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({
    ...
    fundedAt: task.fundedAt?.toISOString() ?? null,
    ...
    slots: task.slots.map((s) => ({
      ...
      evidence: s.evidence.map((e) => ({
        ...
        width: e.width,
        height: e.height,
        transcodedAt: e.transcodedAt?.toISOString() ?? null,
      ...
      dispute: s.dispute
        ? { id: s.dispute.id, status: s.dispute.status, createdAt: s.dispute.createdAt.toISOString() }
        : null,
    })),
